...
function BurdJournals.Tooltips.getExtraInfo(item)
    ...
    local isHiddenCursed = BurdJournals.isHiddenCursedJournal
    ...
    if not hideLootRewardDetails then
        ...
        if journalData.isPlayerCreated ~= true
            and journalData.lootRewardsRevealed ~= true
...
local function applyJournalTooltipPresentation(item)
    ...
    elseif BurdJournals.isHiddenCursedJournal and BurdJournals.isHiddenCursedJournal(item) then
