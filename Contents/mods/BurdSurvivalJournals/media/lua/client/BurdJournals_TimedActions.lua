...
local function tryPlayLootJournalActionSound(character, soundCandidates, trackedSounds)
    ...
    for i = 1, #soundCandidates do
        ...
        if type(soundName) == "string" and soundName ~= "" then
            ...
            if ok and soundId ~= nil and soundId ~= false and soundId ~= 0 then
                if type(trackedSounds) == "table" then
...
local LOOT_JOURNAL_OPEN_PROFILES = {
    cursed = {
        ...
        startSounds = {"BreakMetalItem", "RepairWithWrench", "BuildMetalStructureSmallScrap", "BuildMetalStructureSmall"},
        progressSounds = {
            {progress = 0.38, candidates = {"BreakMetalItem", "RepairWithWrench", "BuildMetalStructureSmallScrap", "BuildMetalStructureSmall"}},
            {progress = 0.76, candidates = {"BreakMetalItem", "RepairWithWrench", "BuildMetalStructureSmallScrap", "BuildMetalStructureSmall"}},
        },
    },
    yuletide = {
        duration = 5.5,
        ...
        startSounds = {"ClothesRipping"},
        progressSounds = {
            {progress = 0.36, candidates = {"ClothesRipping"}},
            {progress = 0.74, candidates = {"ClothesRipping"}},
        },
    },
}
...
function BurdJournals.BindJournalAction:perform()
    ...
    if config.xpAward and config.xpAward > 0 then
        if BurdJournals.applySkillXPCompat then
            BurdJournals.applySkillXPCompat(player, Perks.Tailoring, "Tailoring", config.xpAward, "add")
        elseif BurdJournals.applyXPDeltaCompat then
            BurdJournals.applyXPDeltaCompat(player, Perks.Tailoring, config.xpAward)
        else
